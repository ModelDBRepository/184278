'''
Current clamp demo using Allen Brain's model 472440759.
...
from neuron472440759 import Neuron472440759
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472440759(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472440759; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-88.4749730428)
        h.fcurrent()
